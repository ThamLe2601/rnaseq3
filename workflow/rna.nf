...
complte_ch = Channel.fromPath(params.outdir)
                  .collect()
...
process fastqc {
    ...
    output:
    tuple val(sampleid), path ("*_fastqc.{zip,html}"), emit: report_qc
...
process multiqc {
    ...
    input:
    tuple val(sampleid), path (report_qc)
    path report_trim
    path report_star
    path report_rseqc
...
workflow {
   ...
   multiqc(fastqc.out.report_qc, trim_galore.out.report_trim, star.out.report_star, rseqc.out.report_rseqc, qualimap.out.report_qualimap, picard.out.report_picard, preseq.out.report_preseq, dupradar.out.report_dupradar, featurecount.out.report_featurecount, deseq2.out.report_deseq2, gprofiler.out.report_gprofiler)
}
